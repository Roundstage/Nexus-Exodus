...
- Famine, War, Pestilence and Death are teachable rank auras in a separate Mystic-style slot (`obj/DemonBuff`), outside the custom-buff editor and Combat capstones. Each has a matching Demon-only Hell rank. Their original Four Horsemen bonuses are preserved, including the defensive Pestilence definition, and the four exclude each other plus Mystic/Majin. Only the Daimao/Demon Lord receives Soul Contract; old ordinary-Demon grants are removed when loading a character.
...
- Milestones are independent ranked purchases; none requires another Milestone. The shop uses only the existing catalog and MP budget. Selecting a card previews it; purchasing explicitly spends MP on the server. Momentum, Precision and Fortified Damage share one damage-style slot, with the chosen style and blocked alternatives shown before purchase. Ultimate Buffs belong exclusively to the Combat -> Buffs tree.
...
- Super Ghost Kamikaze Attack uses three homing copies of the caster's current character silhouette with one shared damage budget. Evasive Barrage emits eight independently paced, shared-budget pursuing shots during a fast backward dash, while Sphere of Destruction provides one persistent slow pursuer. Super Explosive Wave is a targetless eight-tile ki burst. Earthquake is a targetless eight-tile physical tremor that ignores flying targets.
