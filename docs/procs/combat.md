...
Invested combat skills use the equal-stat 2.5-factor basic melee hit as their balance baseline. Standard learned attacks target roughly five to seven basic hits, high-commitment charged attacks and barrages target eight to twelve, and self-risk finishers can reach sixteen or more; cheap repeatable Basic Blast remains deliberately below those floors. Sokidan reserves a 24-factor direct-plus-splash budget, Kienzan opens at factor 18 before its per-pierce decay, and Makosen reserves a 24-factor volley budget. Dedicated weapon and Unarmed techniques use larger multipliers appropriate to their cooldown and commitment, while multi-hit sequences preserve adjacency until any configured finishing knockback.

Kikoho separates reward from risk through centralized factors: an equal-stat target takes factor 24 while the caster accumulates only a 6% self-damage pool over time. Startup validation enforces at least a three-to-one target/self ratio so the legacy inversion cannot return.
...
Viltrumite Guard joins the mutually exclusive stance system as a two-second, one-contact frontal defense. Its qualifying hit is reduced to 60% final damage and opens the attacker to a personal Viltrumite follow-up; attacks outside the defender's forward three-direction arc neither receive mitigation nor consume the stance. Punishing Reversal uses the same frontal arc for a one-second melee counter and is checked after weapon Riposte, preventing both counters from resolving against one strike.
...
- Ki Fist and the former Ki Blade are no longer buffs. `KiWeaponTechnique` provides four mutually exclusive invoked forms under Combat -> Ki Weapons, exposes each owned form through the native `Skills` tab as well as the hotbar/player menu, and owns one removable Roleplay Tenkaichi weapon overlay while active. Ki Fist adds 30% of Force and remains compatible with physical weapons; combining it with an equipped sword or forged gloves is called Embue, reports the imbued item in chat, and preserves that equipment's BP reinforcement instead of replacing it with Ki Weapon BP. Ki Sword, Ki Hammer, and Spirit Sword require both swords and forged gloves to be unequipped; equipping either kind of physical weapon deactivates them. Ki Sword adds 70% of Force with sword-like accuracy loss but does not count as a weapon; Ki Hammer adds 100% of Force, counts as a weapon, uses a dedicated RPT sledgehammer appearance, and reduces speed and accuracy; Spirit Sword adds 70% of Force, counts as an energy weapon against Resistance, and weakens as its user's Energy reserve falls.
- The separate proficiency line advances all four techniques through Novice/Normal, Basic/Copper, Adept/Iron, Advanced/Mythril, and Master/Masterwork BP reinforcement. Ki Weapons omit Silver and Auracite, never inherit forged critical chance, cannot be stolen or disarmed, and peak below conventional forged equipment.
...
- Named waves use Combat -> Beam, while Rock Throw/Rock Slide/Rock Tomb use Combat -> Physical. Echoing Slash and Sky Break belong to Combat -> Weapon; their projectiles call `getWeaponCombatDamage()` so equipped-weapon quality and weapon Milestones affect their damage. Both sword waves hold their first frame on the caster's tile before vector movement begins, and Sky Break uses a three-tile explosion radius.
