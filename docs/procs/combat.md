...
- Ki Fist and the former Ki Blade are no longer buffs. `KiWeaponTechnique` provides four mutually exclusive invoked forms under Combat -> Ki Weapons: Ki Fist adds 30% of Force and remains compatible with physical weapons; that combination is called Embue and uses only the physical weapon's BP reinforcement instead of stacking Ki Weapon BP. Ki Sword adds 70% of Force with sword-like accuracy loss but does not count as a weapon; Ki Hammer adds 100% of Force, counts as a weapon, and reduces speed and accuracy; Spirit Sword adds 70% of Force, counts as an energy weapon against Resistance, and weakens as its user's Energy reserve falls.
