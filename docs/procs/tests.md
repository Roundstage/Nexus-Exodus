...
- Verifies every Heran owns one transformation, its activation is one canonical primary state, its male RPT body and green glow apply, reversion restores the original body with zero residual BP, and its additive result equals rather than exceeds standard SSJ1.
...
- Verifies zero-argument Skill-menu dispatch, same-area Sense examination authorization, 50% effective Empowered Defense stats, Frost Nova damage/stun against clientless targets, Earth Prison's complete 40-wall perimeter, Gravity Well turf application/restoration, and visible masterwork enchantment results.
