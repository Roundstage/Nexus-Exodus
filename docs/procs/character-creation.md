...
3. The form constructor performs `prob(1)` once for its server-held Cooler flag, captures up to 24 real hair objects, and creates form-local custom clothing/Frost slot lists. `canDisplayForm()` permits only the host client's uncommitted mob.
4. `GenerateBody()` obtains a current race list, builds five stages, emits lightweight native icon references, and exposes creator-owned imports. Frost form slots show a visual south-facing preview beside each independent catalog/custom selector.
