...
The Viltrumite clothing catalog also includes a white/gray Bulk Jumpsuit tailored to the wider body and a separate Skirt Uniform fitted to the female base. The latter layers over the regular jumpsuit (Soldier Robe); both additions remain independent equipment and preserve their target body's complete animation metadata. Starter equipment uses the greater of its declared appearance priority and its selection-order priority; the browser preview applies the same rule, keeping the skirt above the jumpsuit regardless of submission order. See `ArtSource/Viltrumite/Clothing.md` for generation and visual proofs.
...
- `proc/nexusRaceIconOptions(race_name)` returns the allowed body ID-to-icon catalog for a race; Frost Lord delegates to the full form catalog. Viltrumite and Half-Viltrumite have nine dedicated blue-eyed bodies: Male, Female, and Bulk, each in White, Tan, and Dark (the latter retains the `Black` asset filename). All three body types preserve the supplied DMI artwork and animations; the current non-Bulk Male uses the dedicated Viltrumite base supplied as `Humanbase.dmi`, replacing the earlier generic human artwork. Other humanoid races retain their existing catalog. See `ArtSource/Viltrumite/README.md` for palette sources and regeneration.
...
3. Rare eligibility is resolved server-side. `all_rare_races_common` exposes Legendary Saiyan, Frost Lord, Cooler, and Grand Regent globally; `grantNexusRareRace()` records per-account access to those options or Royal Blood (Argal) in server memory until `commitNexusCharacter()` successfully consumes the matching grant. Royal Blood grants bypass its natural roll and online population limit, expose `viltrumite_royal`, and are consumed only when that trait is successfully created. Failed creation or choosing a different trait preserves the grant. Unused grants survive reconnects but not server restarts. Grand Regents created through either override are saved as non-exclusive rare lineages and do not claim or displace the persistent unique office.
