...
T-Virus Injection, EMP Mine, Land Mine, Orbital Cannon, Drivable Car and Extendo Arm are retired from Science research and manufacturing. Their types remain loadable for existing items and administrator tools, but `isRetiredScienceEquipment()` blocks both catalog display and crafting access even when old global or personal blueprint grants exist. `canUnlockTechnology()` also rejects retired types whose saved blueprint still has `science = 1`. Extendo Arm installation and its runtime range benefit are disabled for this wipe, including previously installed modules.

Nuke, Ki Jammer (`Ki_Field_Generator`), Brain Scrambler and Resource Destroyer are Engineering tier-10 designs, following tier-9 research. They require Technology level 8 (the existing maximum), Engineering specialization, 3300 lifetime Progression XP and their researched node. `isTierTenScienceEquipment()` and `getScienceProgressionTier()` separate the displayed tier from Technology level. Access rechecks the current canonical requirements before considering old blueprint grants or ownership; legacy migration does not automatically award these nodes. Sonic Bomb keeps its separate ordinary research path.
...
- `initializeForgedEquipmentCatalogs()` registers Normal plus seven upgrade materials, twenty-nine weapon designs, twenty-seven armor designs, four glove designs, and three mask designs, including the prior DU appearance catalogs.
- `craftForgedWeapon(style_id)`, `craftForgedArmor(style_id)`, `craftForgedGloves(style_id)`, and `craftForgedMask(style_id)` create Normal equipment after selecting a persistent cosmetic skin.
...
`Time_normalizer` now reduces Time Stop/time-freeze stun duration by 75%. Its regeneration, Energy, and Force penalties remain unchanged; the module no longer depends on the removed long-lived `Frozen` object behavior.
...
- Inputs: obj/A
- Purpose: Validate and fabricate an accessible Science design, then award tier-scaled Technology XP for the completed project.
- Returns: none (implicit).
- Side effects: see implementation.
...
- `initializeArcaneFormulaCatalog()` registers 40 active arcane craft adaptations. Cooking Bag and Magic Fishing Lure have no research nodes or recipes, and `isRetiredScienceEquipment()` also rejects their saved Science blueprint overrides. Existing item types remain loadable.
- Magic Sword, Magic Hammer and Magic Gauntlets create the same Normal material, default appearance, combat statistics and upgradeable equipment families as their Science counterparts. Their crafting route uses Arcane Essence and the existing ore catalysts. Gauntlets are equipable forged gloves and no longer grant passive Magic XP.
- Arcane items use their original Roleplay-Tenkaichi DMI resources and states: six elixirs, Spell Book and the four consumable books, the two-tile Bookcase, Simulation Crystal, Mana Pylon, Magic Vault, Locator, Crystal Ball, Philosopher's Stone and Orb of Mastery. `RTHealthPotion.dmi`, `RTMagicItems.dmi`, `RTManaPylon.dmi`, `RTMagicEffects.dmi` and `RTLab.dmi` are unmodified copies of RPT's `Health Potion.dmi`, `Magic Items.dmi`, `Mana_Pylon.dmi`, `fx.dmi` and `Lab.dmi`; the existing enchantment and Cell Phone sheets already match RPT. Merriment and the Orb retain RPT's additive/subtractive colors.
- `getArcaneItemPreviewIcon()` composes the Bookcase and tinted artifact research previews. `restoreArcaneItemAppearance()` runs after item deserialization to replace only recognized old placeholders, leaving custom saved artwork intact.
