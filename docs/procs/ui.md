...
`HtmlSafety.dm` owns `encodeNexusHtmlAttribute()`, `sanitizeNexusHtml()`, `sanitizeNexusHtmlStyle()`, `buildNexusSafeDocument()`, `normalizeNexusHtmlColor()`, `normalizeNexusChatTextSize()` and `normalizeNexusBrowserUrl()`. Legacy chat is rebuilt from inert formatting tags before it enters `NexusChatBuffer`; stored logs are sanitized on write and read. Quoted browser attributes escape both quote delimiters. The creator review uses DOM text nodes, and command prompts use `list2params()` for skin transport. See [the HTML security review](../HtmlInputSecurity.md) for the allowed subset, compatibility changes and regression coverage.
...
The HudLib chat owns All, Combat, IC, and OOC feeds through the embedded Classic browser, with draggable header/borders, real-height scrolling, channel controls and a permanent Say, OOC, Emote, Logs and CMD footer. Messages use Arial/Helvetica with normal letter case, preserve color and emphasis, and wrap within the panel; pixel fonts remain on titles and buttons. TextSize still controls the readable 12–21px message size. New messages update the existing document and preserve history position. Enter and CMD open the command prompt. Legacy `mob << text` output is retained in All as a System message; sounds, images, browser resources and targeted output continue through BYOND normally.
...
- Classic-only interface (2026-09-24): `hideNexusNativeTabs()` hides and detaches native panes at login, settings load, chat layout application and body swaps. `.Toggle_tabs` redirects to Menu; `ToggleTabs()` keeps its Character shortcut. `mob/Stat()` no longer renders native tabs, and `classicStatPanel()` / `classicStat()` only write into active Classic snapshots. `NexusChatHud` routes chat exclusively to the Classic browser; hiding chat cannot restore a side pane. Existing native-tab selection and visibility settings are ignored without resetting player layouts.

- Browser viewport fitting (2026-09-24): `ClassicHud.js` fits the logical shell to the embedded browser's actual CSS width and height on initialization, every payload, resize and drag reconciliation. BYOND skin dimensions may differ from CSS pixels; reapplying the server's scale after a local resize used to clip the menu's close buttons, scrollbar and footer. This fitting changes only the rendered shell, preserving native geometry, saved layout, logical hotbar columns and action tokens. Browser regression tests exercise 100/125/150/200% viewport mismatches and click the menu's final command, Reset HUD and Close after repeated refreshes.

- Panel/hotbar overlap (2026-09-26): `ClassicHud.applyGeometry()` temporarily hides native hotbar browser controls whose displayed rectangles intersect an open panel. This applies to every bar and collapsed panel header, including after browse, drag, resize and reload. Closing or moving the panel restores the bar without changing saved geometry, open state, slots or hotkeys; bars outside the panel stay visible. CSS stacking cannot order separate native browser controls.

- World quick actions (2026-09-26): World rows show both character and account names and are searchable by either. Admins see permitted Manage Player, Teleport, Summon, Heal, Revive and Inspector buttons. `runContextAction()` re-resolves the row and its available actions at activation; `runTargetContextAction()` requires a connected roster member, current admin authorization and verb ownership, then calls an allowlisted single-target verb with the actual mob. Existing confirmation prompts and logging remain in those verbs. This avoids passing quoted object references through the text command parser. Inspector retains its level-three gate.
...
- Runtime optimization: `shouldRefreshWidget()` skips periodic work for collapsed widgets, refreshes bars/target every 5 ticks and other panels every 10 ticks; explicit refreshes and first payloads bypass this gate. `queueChatRefresh()` coalesces message bursts over two deciseconds. Menus enumerate owned verbs only for Actions, Playtest and Admin, while command execution still revalidates ownership. `buildClassicSlotKeyMap()` scans shortcuts once per bar render instead of once per slot and does not retain stale bindings across renders.
- Bounded incremental chat (2026-09-26): `ChatHistory.dm` stores a `NexusChatBuffer` per client/channel. `appendMessage()` closes legacy markup once, assigns monotonically increasing IDs (including identical text), and prunes to the newest 300 entries, 256 KiB of rendered text, and one hour of age. Messages over 32 KiB use a small notice directing the player to Logs; the original logging path is unchanged. `pruneNexusChatHistory()` expires all channel buffers once per minute even with chat hidden/collapsed, and append/visible refresh also prune. `clearNexusChatHistory()` empties all live channels without resetting IDs or deleting saved logs; the toolbar's CLEAR action calls it.
- `ClassicHud.refreshChat()` compares the active buffer revision, channel, typography and geometry, so traffic in other channels does not resend this feed. `NexusChatBuffer.buildUpdate()` sends only new `{id, html}` entries plus `firstId`/`lastId` retention boundaries. Opening/reloading a browser, switching channels or `chat_sync` requests a snapshot; geometry/font changes send no old message bodies. The browser removes expired nodes, appends a document fragment, keeps retained nodes/selection and compensates scroll position with native scroll anchoring disabled. It keeps at most 300 entry nodes and releases the incoming HTML array after rendering. Browser generation/owner checks also protect clear and resync actions.

- `ClassicHud.dm`: `initializeClassicHud()` owns embedded chat, Sense, target, menu, Stats, Inventory, and Skills controls plus dynamically created hotbar controls; the skin defines the primary bar and both detailed panels so they cannot fall back to white-title native windows. `showClassicWidget()` opens one without replacing the others. The main menu, Inventory, and Skills panels are movable but fixed at the approved 460x680 reference size (or clamped to a smaller viewport), so their controls cannot be broken by resizing. Inventory and Skills retain Use, Bar, and direct Examine actions; the Skills capture accepts every owned non-item object marked `Skill == 1`, including legacy techniques without `hotbar_type`. The primary hotbar is a transparent, player-resizable component centered against the bottom edge; extra horizontal bars auto-size and stack above it without drawing a second enclosing slab. An untouched default bar is populated in legacy key order with the existing non-movement commands (Space Attack, B World Chat, E Use, F Blast, G/H Power, I Injure, L Music, M Build, N Emote), excluding Meditate/Train and explicitly independent hotkeys. J/K remain available without occupying slots. Chat defaults to 540x480. World/Who is a public compact category while administrative world internals remain protected. The larger 1280x820 Progression/Milestone tree is also fixed-size. The manager updates changed payloads, validates owner/generation/action tokens, clamps geometry to the map and saves account layout settings. `refreshChat()` updates only messages without repeated viewport queries. The native-tab button and `legacy` action are removed. `closeClassicLegacy()` only closes any restored legacy window.
- `ClassicData.dm`: `captureClassicData()`, `classicStat()`, `classicStatPanel()` reuse native tab data and visibility rules. Inventory reports its total carried-item count and emits a subject token for every resource/item row, enabling Use, Bar, and Examine. Every owned `/obj/items` may be assigned to a hotbar; objects without a dedicated `Hotbar_use()` fall back to their ordinary `Click()` behavior. Skill and item examination keeps the same embedded browser target: using a skill refreshes its details in place, and Back reconstructs the original Inventory or Skills list instead of opening the detached legacy menu. Capture state is restored even on exceptions. Target data excludes administrative Inspect and only contains readable combat values; data remains available through the appropriate Classic section and owned item/skill actions. Saga/science render-only captures suppress progression mutations.
- `ClassicActionBar.dm`: `initializeClassicSlots()`, `resolveClassicSlot()`, `assignClassicSlot()`, `swapClassicSlots()`, `clearClassicSlot()` maintain dynamically allocated character slot IDs without an application-imposed bar/slot limit; slot keys stay with the slot when actions move. Independent hotkeys store an action binding with `direct = TRUE`; bar initialization and `migrateClassicSlotKeys()` leave them independent. `moveClassicSlotToDirectHotkeys()` copies an available slot action to all of its keys and clears that slot in one saved change. It requires at least one slot key. Clearing, changing or deleting a bar cannot remove these direct bindings. `useClassicSlot()` uses `executeNexusHotkeyAction()`. `getClassicCooldown()` normalizes actual world-time, real-time and calendar deadlines; `getClassicSkillState()` reports cooldown, active, unavailable and resource states. No click-created cooldown timer exists.
...
- `classicStat()` distinguishes the one-argument `stat(object/list/text)` form by its null value argument. DM's `args.len` includes omitted formal parameters, so counting arguments discarded atom references and made Souls/Modules and other legacy rows inert. Numeric zero remains a valid two-argument value.
- `ClassicContextMenu.dm` restores right-click actions on object rows throughout Classic panels. `resolveContextSubject()` requires a currently displayed row; `contextOptions()` exposes owned Soul management, item/skill use, Examine, nearby/owned native object verbs and authorized admin target actions. `runContextAction()` rebuilds that authorization before dispatch and rejects stale references and trade-locked items. Native verbs bind the clicked reference and retain BYOND argument/source checks. Browser menus escape labels, fit scaled panels, and ignore responses after dismissal or category changes.
...
- `HotkeyEditor.dm` and `Browser/HotbarEditor.js` / `.css` provide **Bar slots** and **Hotkeys only** modes with a searchable catalog. Bar mode has a dynamic bar selector and 12-slot pages; **Move to hotkeys only** frees the selected slot while preserving all its keys. Direct mode binds owned skills/items, available actions and authorized native verbs without requiring a bar, and lists independent shortcuts for selection or removal. Real-key capture and a manual selector support Ctrl/Shift/Alt and double tap; `NexusHotkeyEditor.bindKey()` revalidates the selected action and requires acknowledgement of the current binding fingerprint before replacement. `classicTokenForBinding()` exposes the catalog token for an existing direct binding. `resolveNexusHotkeyBinding()` shares `resolveClassicBinding()` for direct actions and resolves slot bindings to their original action, retaining held/release semantics and native ownership/trade/verb checks.
...
- `datum/NexusChatHud` manages chat visibility and routes composition, Logs and CMD actions into the Classic chat. `attachOverlay()` gives the full width to the map and detaches restored native panes. `refreshMessages()` schedules the existing Classic browser update; `closeNexusLegacyChatMarkup()` still isolates each message by closing unmatched formatting tags. The separate side-chat renderer and native-tab attachment callbacks are retired.
- `datum/NexusInterfaceSettings` provides nudge, exact-coordinate, reset and scale controls for the lower bars/Sense stack and main vitals display. Settings → Interface Settings opens it. Layout selection and legacy category toggles have been removed.
...
- `applyNexusInterfaceLayout()` and `hideNexusLegacyInterface()` restore the Classic layout while keeping obsolete output windows detached.
...
- `overlay` is the only supported layout. `normalizeNexusInterfaceLayout()` maps old `side_tabs`, missing and invalid preferences to `overlay`. Account settings persist Classic geometry, viewport reference dimensions and vitals positions; retired native-tab flags and font size are no longer imported or exported.
- Nexus chat maps the legacy `TextSize` preference to a readable 12–21px range in the Classic chat; the default value renders at 13px and inline legacy message markup cannot shrink it below the selected size.
...
- Inputs: None
- Purpose: Restore custom bindings, binding version, keyboard profile, slot-key migration version, and the matching character hotbar backup. Restore legacy IDs only when the character's own legacy table is empty.
- Returns: none (implicit).
- Side effects: see implementation.
...
- Inputs: None
- Purpose: Persist legacy IDs, custom bindings, binding version, keyboard profile, slot-key migration version, and per-character hotbar actions/layout. Reject title-screen or pre-restore writes.
- Returns: none (implicit).
- Side effects: see implementation.

#### mob/proc/ensureNexusHotkeyBackupLoaded
- Called before initialization/migration, binding edits, and legacy bar restoration. Loads the account backup once per character instance even when legacy setup has already populated IDs; repeated initialization preserves current edits and intentional unbinds.
- `readNexusHotkeyBackup(savefile/f)` and `writeNexusHotkeyBackup(savefile/f)` share the production serialization format with reconnect regression tests. An empty modern keymap with a current version remains empty.
...
- Defaults: the shared `getNexusStarterHotkeyTypes()` map retains the basic legacy keys (including J Meditate, K Train, Space Attack and WASD); Ctrl plus each of the eight arrow directions binds its universal Short Dash action. Meditate and Train keep their direct J/K bindings and are excluded from automatic bar population. Unlearned skills are referenced without being granted and become usable when acquired.
- Existing saves: `addNexusStarterHotkeyBindings()` adds only absent keys once. Occupied keys, intentionally empty keyed slots, and actions already rebound to another key are preserved. Newly added keys attach to matching existing bar slots except Meditate/Train and explicit direct bindings, without replacing the bar layout. Existing Meditate/Train slots and slot keys remain until the player uses **Move to hotkeys only**. The existing account backup persists version 3 and the direct-binding marker, preventing later reconnects from undoing unbinds or attaching independent keys to bars.
...
- Purpose: Resolve a slot action, available registered action, owned `can_hotbar` object, or currently authorized native verb at execution time.
...
- Purpose: Render the compact MMO hotbar editor with bar-slot and independent-hotkey modes, a dynamic bar list with creation/deletion, slot addition/removal, 12-slot editor pagination, a searchable skill/item/verb catalog, real key-combination capture, manual key/modifier controls, conflict confirmation, independent shortcut management, and bar layout settings.
- Security: actions are represented by server-issued opaque tokens and revalidated in `Topic()`.

#### datum/NexusHotkeyEditor/Topic
- Purpose: Validate bind, unbind, move-to-direct, layout, clear, import, and close events sent by the in-game browser editor. Binding uses the current mode and validates slot membership or the direct action token before applying a change.
...
- Inputs: mob/A
- Purpose: Display a target's structured public Character Profile through the shared bronze UI, including self-authored title/name, verified account identity, a byte-preserved PNG/JPEG/WEBP/WEBM upload or composite runtime portrait, and allowlisted rich biography markup. WEBM portraits autoplay, loop, remain muted, and fall back to the live sprite on playback failure.
- Returns: none (implicit).
...
- Inputs: None
- Purpose: Save player settings, including the normalized Classic layout, overhead-vitals offsets, and the main-vitals screen position.
- Returns: none (implicit).
- Side effects: mutates game state and/or world resources.
...
- Inputs: None
- Purpose: Open Character through the existing F3 compatibility shortcut. Native tabs are no longer available.
- Returns: none (implicit).
- Side effects: displays the Character panel.
...
- Classic Inventory displays each item's current status beneath its name, beside the action buttons. `obj/items/getNexusInventoryStatus()` supplies both Classic rows and standalone item cards/details: ordinary suffix states (including Equipped/Installed) or Carried; guns use their separate `Equipped` flag and retain the ammunition count. Refreshing after equip/unequip replaces the status.
