...
`isTailGrabbed()` requires a reciprocal grab, a rear hold and an attached tail. `Grabbed_by_tail()` drains 1% maximum Energy per second divided by at least 1 tail-training level, clamps Energy at zero and keeps only one drain worker through rapid release/regrab. Passive recovery, meditation recovery and regenerator Energy recovery stop while the tail is held and resume under their usual rules after release or tail removal. Ordinary grabs do not impose this Energy recovery lock.

`canUseArmStretch()` disables extended grabs for Androids, Bio-Androids, Namekians and Aliens during this wipe, including old saved stretch flags. New Namekians and Bio-Androids no longer receive that flag; Alien Stretchy Arms is removed from AP selection, the Shifter preset and the legacy starter menu. `Extendo_module()` always returns false for this wipe, and the mechanical module cannot be researched, crafted or installed. Target acquisition, target eligibility and the arm movement entry point all enforce the restriction; adjacent grabs and Majin's natural stretching remain available.
...
- `src/Code/PlayerMechanics/ProgressionTrees.dm`
