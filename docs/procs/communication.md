...
Chat text and HTML have separate boundaries: direct scouter/communicator/telepathy identities are escaped, colors and font sizes are validated at use, and shared chat rendering only permits inert formatting. `ChatLog()` and `EmoteLog()` sanitize message bodies and encode source labels before persistence; player/admin viewers also sanitize historical HTML. See [HTML input security](../HtmlInputSecurity.md).
