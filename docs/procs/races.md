...
- `src/Code/Races/Viltrumite/ViltrumiteSkills.dm`
...
`canSelectViltrumiteRoyal()` accepts an account's one-use `Royal Blood` (Argal) grant before checking the cached natural roll and online limit. `canUnlockArgalBloodline()` permits conversion only for a completed, savable standard full Viltrumite who can redistribute stats and is not in a creation/respec transition. `unlockArgalBloodline()` applies the saved Royal lineage and immunity once, then grants or refreshes an existing `/obj/Redo_Stats` at no cooldown. The player uses **Other > Redo Stats** to rebuild their innate Royal attributes and choose the full 52-point allocation using the existing progression-preserving respec. The initial conversion leaves stats, progression, BP, mutations, and inventory intact; it does not rerun race initialization or stack stat bonuses. The Royal lineage and unused respec survive character saves.

`viltrumite_lineage` is the saved rules identifier (`standard`, `royal`, `grand_regent`, `hybrid`, or `royal_hybrid`); `Class` remains the compatible display/admin value. Standard Viltrumites use `2.4 * 0.70 = 1.68` effective creation BP, while the Grand Regent uses `2.4 * 1.375 = 3.30`, takes `0.90` incoming damage, and cannot possess Anger. Full and Half-Viltrumites have Intelligence `1`; their per-character `old_age_on` flag is disabled, so they receive neither natural decline penalties nor old-age death. This is distinct from combat immortality and does not grant death regeneration. Royal Blood increases free attributes and caps rather than passive damage resistance. Normal creation still exposes and atomically binds one persistent Grand Regent office to account, slot, and creation timestamp. An individual rare grant or `all_rare_races_common` instead creates a saved `grand_regent_nonunique` lineage that keeps the same gameplay package without claiming or displacing that office. The audit-logged `setGrandRegent()` succession transfers only the unique office and does not demote test-enabled rare Grand Regents.

Half-Viltrumites appear in creation only while a matching pending family birth exists. Royal Blood, but not the Grand Regent office, can produce Royal Blood descendants or Royal Hybrids. Until the dedicated Viltrum map is added, both Viltrumite race names resolve through `getRaceSpawnName()` to the existing Saiyan spawn. The hidden `scourge_resistance` roll is made once at five percent; Royal Blood, Royal Hybrid, and Grand Regent lineages are unconditionally immune to the final lethal outcome. Resistance does not prevent infection or symptoms: every exposed Viltrumite reaches the terminal Scourge stage before survival is resolved.

`ViltrumiteSkills.dm` defines the shared `Viltrumite Warfare` racial tree for full and Half-Viltrumites. Its assault route contains Viltrumite Rush, Rib Breaker, Relentless Pursuit, Conqueror's Grip, and Meteor Drop; its execution route contains Spear Hand, Nolan's Combination, and the passive Executioner's Hand upgrade; its discipline route contains Viltrumite Guard and Punishing Reversal. All active attacks are unarmed and remain inaccessible to other races even if an object is granted externally. Royal and Grand Regent lineages receive the same techniques and costs rather than an additional skill advantage.

Rush and Rib Breaker create a three-second combat opening owned by the attacking Viltrumite. Rib Breaker additionally reduces Energy recovery to 60% for five seconds without preventing actions. Guard and Punishing Reversal instead create an opening on a successful frontal defense. Only its owner can consume it, granting 15 accuracy to the next racial technique. Viltrumite stagger lasts at most eight ticks: a second application within six seconds is halved, while subsequent applications during the window create setup pressure without extending the stun-lock. Rush also creates a 3.5-second owner-specific pursuit window; Relentless Pursuit consumes it to close distance without damage. Nolan's Combination resolves four individual hits and stops on lost adjacency, block, or evasion. Executioner's Hand never performs an unconditional execution: in Lethal combat it adds 25% Spear Hand damage and heavier bleeding only against a KO or target at 15 Health or lower.
