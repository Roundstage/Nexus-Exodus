...
- `src/Code/Application/Movement/FlashStep.dm`
...
- `src/Code/Application/Movement/PixelMovementInitialization.dm`
...
- `src/Code/Interface/Movement/MovementInputPrompt.dm`
...
- Purpose: Return the normal movement speed cap before tick scaling, including the 16-unit Walk-mode ceiling after ordinary movement penalties.
- Returns: non-negative movement velocity.
- Side effects: none expected.

#### mob/proc/setWalkingMode
- Signature: `setWalkingMode(enabled, announce = TRUE)`
- Purpose: Enable or disable precise normal movement, clamp retained momentum immediately, and optionally notify the player.
- Returns: the resulting Walk-mode state.
- Side effects: updates transient movement state and may display chat/overlay feedback.

#### mob/proc/toggleWalkingMode
- Signature: `toggleWalkingMode()`
- Purpose: Toggle the universal Walk mode used by the remappable hotkey action.
- Returns: the resulting Walk-mode state.
- Side effects: delegates to `setWalkingMode()`.
