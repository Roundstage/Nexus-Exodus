...
const source = fs.readFileSync(path.join(__dirname, 'EarthquakeSheet.png'));
const output = path.resolve(__dirname, '../../src/Icons/Effects/Earthquake.dmi');
...
if (source.readUInt32BE(16) !== 960 || source.readUInt32BE(20) !== 512) throw Error('Unexpected sheet dimensions');
...
console.log(output + ': 24 frames, 160x128, 1.2 seconds, single playback');
