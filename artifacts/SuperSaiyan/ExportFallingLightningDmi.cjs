...
const source = fs.readFileSync(path.join(__dirname, 'FallingLightningSheet.png'));
const output = path.resolve(__dirname, '../../src/Icons/Ki/Electricity/FallingLightning.dmi');
...
if (source.readUInt32BE(16) !== 192 || source.readUInt32BE(20) !== 768) throw Error('Unexpected sheet dimensions');
...
console.log(output + ': four bursts, six 32x192 frames each, 0.3 seconds.');
