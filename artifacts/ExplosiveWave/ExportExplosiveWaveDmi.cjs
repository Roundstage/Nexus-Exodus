...
const source = fs.readFileSync(path.join(__dirname, 'ExplosiveWaveSheet.png'));
const output = path.resolve(__dirname, '../../src/Icons/Effects/ExplosiveWave.dmi');
...
if (source.readUInt32BE(16) !== 800 || source.readUInt32BE(20) !== 640) throw Error('Unexpected sheet dimensions');
const metadata = '# BEGIN DMI\nversion = 4.0\n\twidth = 160\n\theight = 160\nstate = "explosive_wave"\n\tdirs = 1\n\tframes = 20\n\tdelay = ' + Array(20).fill('0.4').join(',') + '\n\tloop = 1\n\trewind = 0\n# END DMI\n';
...
console.log(output + ': 20 frames, 160x160, 0.8 seconds, single playback');
