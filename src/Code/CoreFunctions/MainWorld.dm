...
world
	...
	New()
		...
		AddBlasts()
		World_Status()
		spawn(50) name = "Nexus Exodus" //trying to fix the "3.5 simulator" bug where it keesp renaming it to that at the top of the window
...
mob/proc/choose_alignment()
	var/choice = alert(src,"Choose your character's alignment. Good people are unable to kill or steal from other \
	...
	setInitialAlignment(choice)

mob/proc/setInitialAlignment(choice)
	if(!(choice in list("Good", "Evil"))) choice = "Good"
	ChangeAlignment(choice)
	if(choice == "Evil") Detect_good_people()
