...
mob/proc
	StatRaceCapped(s)
		...
		switch(s)
			...
			if("Offense") if(offmod >= NexusGeneralCombatStatCap()) return 1
			if("Defense") if(defmod >= NexusGeneralCombatStatCap()) return 1
...
mob/proc/Racial_Stats(mob/P,Start_Redo_Stats=1,modless_check=1,auto_allocate=0,list/stat_allocation) //If P, P gets to do the stats on this mob.
	...
	ApplyRaceBuild(Race)
	if(!stat_allocation) applyCharacterMutations()

	Max_Points=Points
	Set_Minimum_Stats()
	if(stat_allocation)
		if(!applyNexusStatAllocation(stat_allocation)) return FALSE
		applyCharacterMutations()
	else if(auto_allocate)
		autoAllocateCharacterStats()
	else if(!dbz_character&&Start_Redo_Stats)
		if(P.client)
