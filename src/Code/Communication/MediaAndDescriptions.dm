...
datum/NexusPlayerDescriptionEditor
	...
	proc/buildHtml(list/portrait_resources)
		...
		var/custom_art_is_video = using_custom_art && owner.player_profile_art_format == "webm"
		var/current_portrait = using_custom_art && !custom_art_is_video ? portrait_resources["custom"] : (portrait_resources ? portrait_resources["[current_direction]"] : "")
		var/custom_video_attributes = custom_art_is_video ? " src='[portrait_resources["custom"]]' style='display:block'" : " style='display:none'"
		var/current_image_attributes = custom_art_is_video ? " style='display:none'" : ""
		...
		if(has_stored_art)
			...
			source_actions += "<button class='hud-button submit-action' type='submit' onclick=\"return setProfileAction('upload_art')\">REPLACE MEDIA</button><button class='hud-button danger submit-action' type='submit' onclick=\"if(!confirm('Permanently delete the custom art for this character slot?'))return false;return setProfileAction('delete_art')\">DELETE ART</button>"
		else
			source_actions = "<button class='hud-button submit-action' type='submit' onclick=\"return setProfileAction('upload_art')\">UPLOAD MEDIA</button>"
		...
		function setProfileAction(actionValue){document.getElementById('profileAction').value=actionValue;return true;}function lockProfileForm(){var buttons=document.getElementsByClassName('submit-action');for(var i=0;i<buttons.length;i++)buttons\[i].disabled=true;return true;}var customPortrait='[portrait_resources["custom"]]',customPortraitFormat='[owner ? owner.player_profile_art_format : ""]',usingCustom=[using_custom_art ? "true" : "false"];function profilePortraitFallback(media){if(!usingCustom||media.getAttribute('data-fallback')==='1')return;usingCustom=false;media.setAttribute('data-fallback','1');var image=document.getElementById('portrait'),video=document.getElementById('portraitVideo'),direction=document.getElementById('portraitDirection').value;video.pause();video.style.display='none';video.removeAttribute('src');image.style.display='block';image.setAttribute('data-fallback','1');image.src=portraitResources\[direction\]||'';document.getElementById('portraitFrame').className='portrait sprite-art hud-sprite';document.getElementById('portraitSource').innerHTML='LIVE SPRITE FALLBACK';}updatePortrait=function(){var direction=document.getElementById('portraitDirection').value,image=document.getElementById('portrait'),video=document.getElementById('portraitVideo');if(usingCustom&&customPortraitFormat==='webm'){image.style.display='none';video.style.display='block';video.src=customPortrait;video.play();return;}video.pause();video.style.display='none';image.style.display='block';image.src=usingCustom?customPortrait:(portraitResources\[direction\]||'[current_portrait]');};updatePortrait();
		</script></body></html>"}
		return replacetext(rendered_html, "PNG, JPEG, or WEBM", profile_media_formats)
