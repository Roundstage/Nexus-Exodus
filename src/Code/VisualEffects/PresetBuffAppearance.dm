...
datum/PlayerAppearanceManager/proc/syncPresetBuffs()
	...
	for(var/obj/skill in active_skills)
		...
		for(var/list/visual in catalog["[skill.type]"])
			index++
			setSlot("preset_buff:\ref[skill]:[index]", "preset_buff", APPEARANCE_PRIORITY_BUFF, visual[1], skill, "", visual[2], visual[3], visual[4], visual[5])

// Buff flags and owned skills survive saves; rendered image handles do not.
datum/PlayerAppearanceManager/proc/syncSkillBuffs()
	clearCategory("buff")
	if(!owner) return
	syncSkillBuffIcon("fire_fist", 'src/Icons/VFX/FlamingFists.dmi', owner.isFireFist)
	syncSkillBuffIcon("saiyan_power", 'src/Icons/VFX/SaiyanPower.dmi', owner.usingSaiyanPower)
	for(var/obj/Majin/skill in owner)
		syncSkillBuffIcon("majin:\ref[skill]", skill.icon, owner.ismajin, skill)
	for(var/obj/Limit_Breaker/skill in owner)
		syncSkillBuffIcon("limit_breaker:\ref[skill]", skill.icon, owner.limit_breaker_on, skill)
	for(var/obj/Buff/buff in owner)
		var/index = 0
		for(var/buff_icon in buff.buff_overlays)
			if(buff_icon == "Cancel") continue
			index++
			syncSkillBuffIcon("custom:\ref[buff]:[index]", buff_icon, !!buff.suffix, buff)

datum/PlayerAppearanceManager/proc/syncSkillBuffIcon(slot_key, appearance_icon, active, obj/source)
	if(!appearance_icon) return
	var/image/reference_appearance = image(icon = appearance_icon)
	var/list/stale = list()
	for(var/appearance_value in owner.overlays)
		if(appearance_value:icon != reference_appearance.icon) continue
		// Preserve other layers even if a custom buff reuses the powerup aura's resource.
		// Only active buffs own legacy raw icons; inactive custom buffs must not erase cosmetics.
		if(appearance_value:layer == PLAYER_BUFF_LAYER || (active && appearance_value:layer == FLOAT_LAYER)) stale += appearance_value
	owner.overlays -= stale
	if(active) setSlot("buff:[slot_key]", "buff", APPEARANCE_PRIORITY_BUFF, appearance_icon, source, "")
