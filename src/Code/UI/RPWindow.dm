...
datum/NexusEmoteEditor
	...
	proc/buildHtml()
		...
		var/layout_safety_css = "html,body{max-width:100%;overflow:hidden}.shell,.header,.toolbar,.workspace,.pane,.preview,.emote-card,.footer,.title,.identity,.hint{min-width:0;max-width:100%}.workspace{grid-template-columns:minmax(0,1fr) minmax(0,1fr);overflow:hidden}.pane,.emote-card{overflow:hidden}.editor{min-width:0;min-height:0;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all}.preview,.preview *,.title b,.title span,.identity,.hint{overflow-wrap:anywhere;word-break:break-word}.post,.close{flex:0 0 auto}@media(max-width:760px){.workspace{grid-template-columns:minmax(0,1fr)}}"
		return {"<!doctype html><html><head><meta charset='utf-8'><title>Emote Editor</title><style>[getNexusRpgBrowserCss()][layout_safety_css]
...
mob/verb/ViewDescription(mob/A)
	...
	var/custom_art_is_video = using_custom_art && A.player_profile_art_format == "webm"
	...
	if(length(portrait_resource))
		if(custom_art_is_video)
			portrait_html = "<div id='profilePortraitFrame' class='portrait custom-art'><img id='profilePortraitFallback' src='[live_portrait_resource]' style='display:none' alt='Character portrait'><video id='profilePortraitVideo' src='[portrait_resource]' autoplay loop muted playsinline aria-label='Animated character portrait' onerror='nexusProfilePortraitFallback(this)'></video></div>"
		else
			portrait_html = "<div id='profilePortraitFrame' class='portrait [using_custom_art ? "custom-art" : "sprite-art hud-sprite"]'><img id='profilePortraitImage' src='[portrait_resource]' alt='Character portrait' onerror='nexusProfilePortraitFallback(this)'></div>"
	...
	.shell{max-width:900px;margin:0 auto}.profile{min-height:560px;padding:22px}.profile-head{display:grid;grid-template-columns:260px 1fr;gap:22px;align-items:center;padding-bottom:18px;margin-bottom:20px;border-bottom:2px solid #715735}.portrait{width:260px;height:280px;padding:10px;display:flex;align-items:center;justify-content:center}.portrait img,.portrait video{width:auto;height:auto;max-width:236px;max-height:256px;object-fit:contain}.portrait.sprite-art img{image-rendering:pixelated}.portrait.custom-art img{image-rendering:auto}.identity{display:flex;flex-direction:column;gap:7px}.profile-title{font-size:12px}.profile-name{font-size:25px}.portrait-source{font-size:9px}.biography-title{margin:0 0 14px;padding:9px 11px;font-size:13px}.biography{line-height:1.7;overflow-wrap:anywhere}.empty{font-style:italic}@media(max-width:650px){.profile-head{grid-template-columns:1fr}.portrait{margin:0 auto}}
	</style></head><body class='nexus-hud'><main class='shell hud-shell'><section class='profile hud-card'><header class='profile-head'>[portrait_html]<div class='identity'><span class='hud-label'>PUBLIC CHARACTER PROFILE / SELF-AUTHORED</span>[title_html]<b class='profile-name hud-title'>[profile_name]</b><span id='profilePortraitSource' class='portrait-source hud-muted'>PORTRAIT: [portrait_source]</span></div></header><h2 class='biography-title hud-section-title'>BIOGRAPHY</h2><div class='biography'>[description_html]</div></section></main><script>function nexusProfilePortraitFallback(media){if(!media||media.getAttribute('data-fallback')==='1')return;media.setAttribute('data-fallback','1');media.onerror=null;var fallback=document.getElementById('profilePortraitFallback');if(media.tagName&&media.tagName.toLowerCase()==='video'&&fallback){media.pause();media.style.display='none';media.removeAttribute('src');fallback.style.display='block';}else{media.src='[live_portrait_resource]';}var frame=document.getElementById('profilePortraitFrame');if(frame)frame.className='portrait sprite-art hud-sprite';var label=document.getElementById('profilePortraitSource');if(label)label.innerHTML='PORTRAIT: LIVE SPRITE FALLBACK';}</script></body></html>"}
