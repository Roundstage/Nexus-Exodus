...
datum/NexusAdminInspector
	...
	proc/buildListHtml(variable_name)
		...
		for(var/entry in value_list)
			...
			if(istext(entry)) associated_value = value_list[entry]
		...
		return {"<!doctype html><html><head><meta charset='utf-8'><title>Admin List Inspector</title><style>[getNexusRpgBrowserCss()]
	...
	proc/buildHtml()
		...
		for(var/category in category_order)
			for(var/variable_name in variable_names)
				...
				rows_html += "<tr data-category='[category]' data-search='[encodeNexusHtmlAttribute(search_key)]'><td><span class='category'>[category]</span></td><td><a href='byond://?src=\ref[src]&action=edit&var=[variable_name]'>[html_encode(variable_name)]</a></td><td>[display_html]</td><td><small>[html_encode(runtime_type)]</small></td></tr>"
		return {"<!doctype html><html><head><meta charset='utf-8'><title>Admin Inspector</title><style>[getNexusRpgBrowserCss()]
