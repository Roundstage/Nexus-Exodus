...
mob/proc/buildCharacterSheetHtml(portrait_resource)
	...
	var/admin_button = IsAdmin() ? "<a class='top-button admin' href='byond://?src=\ref[src]&action=open_admin_inspector'>ADMIN INSPECTOR</a>" : ""
	...
	*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:#071018;color:#eef6f2;font:13px Arial,sans-serif}body{background:radial-gradient(circle at 70% 10%,#244634 0,#10291f 30%,#09151a 68%,#050a0e 100%)}
	...
	<div class='topbar'><div class='title'><b>CHARACTER</b><span>Detailed DU assessment</span></div><div class='mode [sparring_mode == LETHAL_COMBAT ? "lethal" : ""]'>[lethal_label]</div><div class='mode [rp_mode ? "rp" : ""]'>RP Mode [rp_mode ? "ON" : "OFF"]</div>[admin_button]<a class='top-button' href='byond://?src=\ref[src]&action=refresh_character_sheet'>REFRESH</a></div>
...
mob/proc/showCharacterSheet()
	...
	var/portrait_resource = "nexus_character_[ckey(key)]_[world.time].png"
	var/icon/portrait_icon = icon(src.icon, src.icon_state, SOUTH)
	src << browse_rsc(portrait_icon, portrait_resource)
	src << browse(buildCharacterSheetHtml(portrait_resource), "window=NexusCharacter;size=1180x760;can_resize=true")
