...
proc/runNexusProfileArtSmokeTests()
	...
	nexusSmokeAssert(isNexusProfileArtUploadName("portrait.png") && isNexusProfileArtUploadName("PORTRAIT.PNG") && isNexusProfileArtUploadName("portrait.jpg") && isNexusProfileArtUploadName("portrait.JPEG") && isNexusProfileArtUploadName("portrait.webm") && isNexusProfileArtUploadName("PORTRAIT.WEBM") && !isNexusProfileArtUploadName("portrait.webm.exe") && !isNexusProfileArtUploadName("portrait.gif") && !isNexusProfileArtUploadName("portrait.dmi"), "profile-art filename preflight does not enforce the PNG/JPEG/WEBM allowlist")
...
proc/runStartupSmokeTests(soul_contract_count_before)
	...
	nexusSmokeAssert(findtext(profile_builder_html, "name='profile_name'") && findtext(profile_builder_html, "name='profile_title'") && findtext(profile_builder_html, "name='portrait_direction'") && findtext(profile_builder_html, "name='generation'") && findtext(profile_builder_html, "The Azure Pilgrim") && findtext(profile_builder_html, "right.png") && findtext(profile_builder_html, "wrap('\[b]','\[/b]')") && findtext(profile_builder_html, "UPLOAD MEDIA") && findtext(profile_builder_html, "PNG, JPEG, or WEBM") && findtext(profile_builder_html, "3840&times;2160") && findtext(profile_builder_html, "EXIF/GPS") && findtext(profile_builder_html, "<video id='portraitVideo'") && findtext(profile_builder_html, "profilePortraitFallback") && findtext(profile_builder_html, "setProfileAction('upload_art')") && !findtext(profile_builder_html, "<img src='http"), "the profile builder omits structured identity, WEBM/4K upload, privacy/fallback, stale-form, portrait, or rich-text controls")
	del(profile_builder)
