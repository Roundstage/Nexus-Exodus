...
proc/runViltrumiteStartupSmokeTests()
	...
	del(parent)
	del(birth_candidate)
	var/mob/NexusSmokeTest/mate_requester = new
	var/mob/NexusSmokeTest/mate_target = new
	mate_requester.gender = "male"
	mate_target.gender = "female"
	var/obj/Mate/requester_mate = new(mate_requester)
	nexusSmokeAssert(!mate_target.canReceiveMateRequest(mate_requester), "Mate request can be sent to a player without the Mate ability")
	var/obj/Mate/target_mate = new(mate_target)
	nexusSmokeAssert(mate_target.canReceiveMateRequest(mate_requester), "eligible Mate request was rejected")
	mate_target.KO = TRUE
	nexusSmokeAssert(!mate_target.canReceiveMateRequest(mate_requester) && !mate_target.Can_Mate(), "Mate request can be sent to a KO player")
	mate_target.KO = FALSE
	// BYOND real-time timestamps lose sub-minute precision at this date.
	target_mate.Next_Use = world.realtime + 600
	nexusSmokeAssert(!mate_target.canReceiveMateRequest(mate_requester), "Mate request bypassed the recipient cooldown")
	nexusSmokeAssert(!text2path("/mob/proc/Mate_Graphics"), "removed Mate animation proc is still compiled")
	del(requester_mate)
	del(target_mate)
	del(mate_requester)
	del(mate_target)
...
proc/runStartupSmokeTests(soul_contract_count_before)
	...
	var/datum/NexusEmoteEditor/emote_layout_contract = new
	var/emote_layout_html = emote_layout_contract.buildHtml()
	nexusSmokeAssert(findtext(emote_layout_html, "minmax(0,1fr)") && findtext(emote_layout_html, "overflow-wrap:anywhere") && findtext(emote_layout_html, "word-break:break-all"), "emote editor does not contain long unbroken text safely")
	del(emote_layout_contract)
	...
	nexusSmokeAssert(unauthenticated_shortcut_types.len == 10 && (/obj/NexusHud/ShortcutButton/World in unauthenticated_shortcut_types), "non-admin shortcut HUD does not expose the public World panel")
	...
	var/icon/shortcut_bar_icon = getNexusShortcutBarIcon(10)
	nexusSmokeAssert(shortcut_button_icon.Width() == 26 && shortcut_button_icon.Height() == 26, "shortcut HUD button has invalid dimensions")
	nexusSmokeAssert(shortcut_bar_icon.Width() == 288 && shortcut_bar_icon.Height() == 34, "shortcut HUD backing strip has invalid dimensions")
	...
	skill_examine_contract.embedded_widget = "skills"
	nexusSmokeAssert(skill_examine_contract.getBrowserOptions() == "window=mapwindow.classic_skills", "embedded Skills examine can reopen the detached legacy menu")
	skill_examine_contract.embedded_widget = null
	nexusSmokeAssert(findtext(skill_examine_contract.getBrowserOptions(), "window=NexusPlayerMenu"), "standalone player menu lost its own browser target")
	...
	nexusSmokeAssert(findtext(rendered_skill_contract, "nexus_skill_") && findtext(rendered_inventory_contract, "nexus_atom_") && !findtext(rendered_inventory_contract, "nexus_pixel_"), "Skills lost its dedicated artwork or Inventory lost its runtime item sprite")
	...
	nexusSmokeAssert(rock_projectile_contract.in_use && !rock_projectile_contract.blast_caches && !rock_projectile_contract.Can_Home && rock_projectile_contract.strength_scaled == FALSE, "rock projectile defaults do not activate straight-line flight safely")
	...
	nexusSmokeAssert(vitals_panel.screen_loc == "LEFT:0,BOTTOM:0" && vitals_panel.plane == NEXUS_FIXED_HUD_PLANE && (vitals_panel.appearance_flags & RESET_TRANSFORM), "main vitals HUD is not flush with the lower-left corner or isolated from character scaling")
	...
	nexusSmokeAssert(active_modifiers_icon.Width() == 296 && active_modifiers_icon.Height() == 56, "active modifier strip has invalid dimensions")
	nexusSmokeAssert(!text2path("/obj/NexusHud/VitalRow/Power"), "redundant horizontal power bar still exists")
	nexusSmokeAssert(findtext(vitals_panel.willpower_row.detail_text.rendered_text, "50%") && vitals_panel.willpower_row.pixel_y > vitals_panel.health_row.pixel_y, "Willpower percentage is not rendered above Health")
	nexusSmokeAssert(vitals_panel.willpower_row.primary_text.rendered_text == "WP" && vitals_panel.health_row.primary_text.rendered_text == "HP" && vitals_panel.energy_row.primary_text.rendered_text == "KI" && vitals_panel.stamina_row.primary_text.rendered_text == "STA", "compact main vitals HUD is missing its status labels")
	nexusSmokeAssert(findtext(vitals_panel.energy_row.detail_text.rendered_text, "(8000) 100%") && vitals_panel.energy_row.detail_alignment == "right", "Energy does not use the (ki) percentage% format")
	nexusSmokeAssert(!findtext(vitals_panel.power_readout.rendered_text, "<br>"), "power readout still renders duplicate lines")
	...
	nexusSmokeAssert(vitals_panel.active_modifiers_readout.alpha == 255 && findtext(vitals_panel.active_modifiers_readout.header_text.text_value, "Focus") && findtext(vitals_panel.active_modifiers_readout.first_row_text.text_value, "BP 1.18x"), "active modifier strip did not render the live Focus summary")
	...
	nexusSmokeAssert(!findtext(vitals_panel.active_modifiers_readout.header_text.text_value, "<br>") && !findtext(vitals_panel.active_modifiers_readout.first_row_text.text_value, "<br>"), "active modifier strip still depends on unreliable multiline maptext")
	...
	var/list/editor_actions = warp_player.buildClassicActionCatalog()
	var/list/editor_action_names = list()
	for(var/list/editor_action in editor_actions) editor_action_names += editor_action["name"]
	nexusSmokeAssert(!("Zanzoken: North" in editor_action_names), "hotkey editor lists Zanzoken without its skill")
	nexusSmokeAssert(("Short Dash: North" in editor_action_names) && ("Short Dash: Northwest" in editor_action_names) && ("Cycle Target" in editor_action_names) && ("Toggle Walk" in editor_action_names), "hotkey editor does not expose all universal movement and targeting actions")
...
proc/runNexusDestructionAuraVisualSmoke(mob/skill_acceleration_test)
	...
	aura_visual_test.showAura(skill_acceleration_test)
	var/obj/Screen_Indicator/sense_indicator_test = new
	sense_indicator_test.target = skill_acceleration_test
	sense_indicator_test.SenseArrowMatchAppearance()
	nexusSmokeAssert(!(aura_visual_test.aura_flames in sense_indicator_test.overlays) && !(aura_visual_test.aura_field in sense_indicator_test.underlays), "Sense indicator copied the target's destruction aura visuals")
	var/icon/sense_readout_icon = getNexusSenseReadoutIcon()
	nexusSmokeAssert(sense_readout_icon && sense_readout_icon.Width() == 1 && sense_readout_icon.Height() == 1 && !sense_readout_icon.GetPixel(1, 1), "Sense readout does not use an explicit transparent anchor icon")
	del(sense_indicator_test)
