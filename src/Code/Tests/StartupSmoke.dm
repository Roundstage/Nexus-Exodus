...
proc/runStartupSmokeTests(soul_contract_count_before)
	...
	var/icon/lighting_mask = icon('TorchLightCircle.dmi')
	...
	nexusSmokeAssert(lighting_mask.Width() == 517 && lighting_mask.Height() == 517, "lighting mask is corrupt or has invalid dimensions")
	lighting_emitter.configureNexusEmitter("#ffffff", 4, 200, 'TorchLightCircle.dmi', TRUE)
	nexusSmokeAssert(lighting_emitter.core_visual && lighting_emitter.range_tiles == 4 && lighting_emitter.alpha < lighting_emitter.core_visual.alpha, "layered glow does not have a compact core and softer ranged falloff")
	nexusSmokeAssert(round(getNexusGlowRangeScale(4) * 517 / (world.icon_size * 2), 0.1) == 4, "glow range no longer maps to tile radius")
	...
	var/obj/NexusLighting/Emitter/action_glow = lighting_owner.setNexusActionGlow("#ffffff", 4, 255)
	nexusSmokeAssert(action_glow && action_glow in lighting_owner.vis_contents, "independent beam/action glow was not attached to its owner")
