#define NEXUS_PROGRESSION_VERSION 1
#define NEXUS_PASSIVE_PROGRESSION_INTERVAL 18000
...
proc/getProgressionTierLifetimeRequirement(tier)
	...
	if(tier > tier_requirements.len) return tier_requirements[tier_requirements.len]
	return tier_requirements[tier]

proc/getProgressionBaseActiveExperiencePerHour()
	return 36000 / NEXUS_PASSIVE_PROGRESSION_INTERVAL
...
proc/initializeProgressionCombatCatalog()
	...
	for(var/ki_type in getTenkaichiSpecialStyleAttackTypes())
		...
		registerProgressionSkillType(ki_type, "Ki", ki_tier, 4 + ki_tier * 5)
	...
	for(var/branch in magic_branches)
		...
		for(var/magic_level = 1, magic_level <= magic_level_thresholds.len, magic_level++)
			...
			for(var/magic_id in magic_research_catalog)
				...
				node.icon_file = initial(magic_node.reward_type:icon)
				node.icon_state = initial(magic_node.reward_type:icon_state)
	...
	for(var/branch in list("Foundation", "Engineering", "Robotics", "Genetics"))
		...
		for(var/required_level = 1, required_level <= technology_level_thresholds.len, required_level++)
			...
			for(var/obj/technology in tech_list)
				...
				var/technology_type_text = "[technology.type]"
				var/node_id = "science_[md5(technology_type_text)]"
				var/parent_id = previous_tier[((current_tier.len) % previous_tier.len) + 1]
...
proc/initializeProgressionProfessionCatalog()
	...
	var/datum/ProgressionNode/smith_root = createProgressionNode("smithing_apprentice", "Forge Apprentice", "Unlocks Copper weapons, armor and pickaxes.", "Smithing", "Forgecraft", 1, 3)
...
mob/proc/applyProgressionNodeReward(datum/ProgressionNode/node, announce = TRUE)
	...
	switch(node.reward_kind)
		...
		if("technology")
			...
			for(var/obj/technology in tech_list)
				if(technology.type == node.reward_type && !(technology in individual_science_items))
...
mob/proc/tryAwardProgressionRoleplaySession()
	...
	var/reward = NEXUS_ROLEPLAY_SESSION_REWARD * progression_roleplay_session_multiplier
...
mob/proc/updatePassiveProgression()
	if(!client || !playerCharacter) return
	if(progression_last_passive_realtime <= 0)
		progression_last_passive_realtime = world.realtime
		return
	if(client.inactivity > 3000)
		progression_last_passive_realtime = world.realtime
		return
	var/interval = NEXUS_PASSIVE_PROGRESSION_INTERVAL * max(0.7, 1 - getMilestoneRank("patient_growth") * 0.1)
	var/elapsed = world.realtime - progression_last_passive_realtime
	if(elapsed < interval) return
	var/awards = min(12, floor(elapsed / interval))
	progression_last_passive_realtime += awards * interval
	if(awards > 0) gainProgressionExperience(awards, "active time", announce = FALSE, source_kind = "passive")
...
mob/proc/syncProgressionTrees(silent = TRUE)
	...
	if(progression_tree_version < NEXUS_PROGRESSION_VERSION)
		if(isnum(Experience) && Experience > 0)
			progression_experience += Experience
			progression_lifetime_experience += Experience
			Experience = 0
		for(var/node_id in progression_node_catalog)
			var/datum/ProgressionNode/node = progression_node_catalog[node_id]
			if(node.reward_kind == "skill" && node.reward_type && hasExactProgressionRewardObject(node.reward_type)) progression_nodes_owned[node.id] = max(1, getProgressionNodeRank(node.id))
				...
				for(var/obj/technology in tech_list)
					...
					if((technology in GLOBAL_SCIENCE_TAB_ITEMS) || (technology in individual_science_items) || canUnlockTechnology(technology)) progression_nodes_owned[node.id] = 1
					break
...
datum/NexusProgressionTreeWindow
	...
	proc/buildNodeIcon(datum/ProgressionNode/node)
		if(!node.icon_file) return "<span class='fallback'>[uppertext(copytext(node.name, 1, 2))]</span>"
		var/icon_key = "[node.icon_file]|[node.icon_state]"
		var/resource_name = "progression_[md5(icon_key)].png"
		if(!islist(owner.client.progression_browser_icon_resources)) owner.client.progression_browser_icon_resources = list()
		if(!owner.client.progression_browser_icon_resources[resource_name])
	...
	proc/buildProgressionGraphNode(datum/ProgressionNode/node, node_x, node_y)
		...
		var/capstone_class = node.exclusive_group == "ultimate_buff" ? "capstone" : ""
		return "<a class='tree-node [state] [capstone_class]' style='left:[node_x]px;top:[node_y]px' href='[action]'><span class='node-tier'>T[node.tier]</span><span class='node-icon'>[buildNodeIcon(node)]</span><b>[html_encode(node.name)]</b><span class='node-meta'>[rank]/[node.max_rank] · [node.cost] XP</span><span class='node-tip'><strong>[html_encode(node.name)]</strong><small>[html_encode(node.description)]</small><em>[html_encode(requirement)]</em></span></a>"
	...
	proc/buildMilestoneGraphNode(datum/MilestoneDefinition/milestone, node_x, node_y)
		...
		var/initial_letter = uppertext(copytext(milestone.name, 1, 2))
		return "<a class='tree-node [state]' style='left:[node_x]px;top:[node_y]px' href='[action]'><span class='node-tier'>T[milestone.tier]</span><span class='node-icon'><span class='fallback'>[initial_letter]</span></span><b>[html_encode(milestone.name)]</b><span class='node-meta'>[rank]/[milestone.max_rank] · [milestone.cost] MP</span><span class='node-tip'><strong>[html_encode(milestone.name)]</strong><small>[html_encode(milestone.description)]</small><em>[html_encode(status_text)]</em></span></a>"
	...
	proc/buildMilestoneList()
		...
		for(var/datum/MilestoneDefinition/milestone in entries)
			...
			list_html += "<a class='milestone-card [state]' href='[action]'><span class='milestone-branch'>[html_encode(uppertext(milestone.branch))]</span><strong>[html_encode(milestone.name)]</strong><small>[html_encode(milestone.description)]</small><span class='milestone-status'>[rank]/[milestone.max_rank] · [milestone.cost] MP · [html_encode(status_text)]</span></a>"
		list_html += "</div>"
		return list_html
	...
	proc/getAvailableBranches(category_name = null)
		...
		if(category_name == "Milestones")
			...
			for(var/milestone_id in milestone_catalog)
				...
				if(!(milestone.branch in branches)) branches += milestone.branch
		else
			initializeProgressionTreeCatalog()
			for(var/node_id in progression_node_catalog)
				var/datum/ProgressionNode/node = progression_node_catalog[node_id]
	...
	proc/getDefaultBranch(category_name)
		...
		switch(category_name)
			...
			if("Milestones") preferred_branch = "Resolve"
	...
	proc/buildTreeNavigation()
		...
		for(var/branch_name in getAvailableBranches())
			...
			branch_html += "<a class='branch-tab [state]' href='byond://?src=\ref[src]&action=filter&id=[url_encode(branch_name)]'>[html_encode(uppertext(branch_name))]</a>"
		var/clear_search = search_query ? "<a class='clear-search' href='byond://?src=\ref[src]&action=search&q='>CLEAR</a>" : ""
		var/search_status = search_query ? "Searching every [html_encode(category)] branch for <b>[html_encode(search_query)]</b>" : "Showing <b>[html_encode(branch_filter)]</b>; choose another branch without rebuilding the whole catalog"
		var/tier_html = ""
		var/tier_label = "JUMP"
		if(category == "Milestones") tier_label = "NO PREREQUISITES"
		else for(var/tier_number = 1, tier_number <= 10, tier_number++) tier_html += "<a href='#' onclick='jumpTier([tier_number]);return false'>T[tier_number]</a>"
		return "<div class='tree-tools'><form onsubmit='return runTreeSearch()'><input id='tree-search' maxlength='60' value='[html_encode(search_query)]' placeholder='Search skills, research or branch...'><button type='submit'>SEARCH</button>[clear_search]</form><div class='tier-jumps'><span>[tier_label]</span>[tier_html]</div><div class='search-status'>[search_status]</div></div><div class='branch-tabs'>[branch_html]</div>"
	...
	proc/buildGraph()
		...
		for(var/datum/entry in entries)
			...
			var/is_global_root = !is_milestone_graph && (!islist(entry_prerequisites) || !entry_prerequisites.len)
			if(is_global_root) continue
		...
		for(var/branch in branches)
			...
			var/branch_height = max(150, largest_stack * 104 + 46)
		...
		var/canvas_width = max(980, 210 + max_tier * 190)
		var/list/positions = list()
		var/list/slot_indexes = list()
		var/root_count = 0
		for(var/datum/entry in entries)
			var/list/entry_prerequisites = entry:prerequisites
			if(!is_milestone_graph && (!islist(entry_prerequisites) || !entry_prerequisites.len)) root_count++
		var/root_index = 0
		for(var/datum/entry in entries)
			...
			var/list/entry_prerequisites = entry:prerequisites
			var/is_global_root = !is_milestone_graph && (!islist(entry_prerequisites) || !entry_prerequisites.len)
			var/node_x = 70 + (entry_tier - 1) * 190
			var/node_y
			if(is_global_root)
				root_index++
				node_y = canvas_height / 2 - 42 + (root_index - (root_count + 1) / 2) * 104
			else
				var/entry_branch = "[entry:branch]"
				var/count_key = "[entry_branch]|[entry_tier]"
				...
				node_y = branch_centers[entry_branch] - ((stack_count - 1) * 104) / 2 + slot_index * 104 - 42
			positions[entry_id] = list("x" = round(node_x), "y" = round(node_y))

		var/lane_html = ""
		for(var/branch in branches)
			lane_html += "<div class='branch-lane' style='top:[round(branch_tops[branch])]px;height:[round(branch_heights[branch])]px'><span>[html_encode(branch)]</span></div>"
		var/tier_html = ""
		for(var/tier_number = 1, tier_number <= max_tier, tier_number++)
			var/tier_x = 70 + (tier_number - 1) * 190
		...
		for(var/datum/entry in entries)
			...
			for(var/prerequisite_id in entry_prerequisites)
				...
				var/x1 = parent_position["x"] + 122
				var/y1 = parent_position["y"] + 39
				var/x2 = entry_position["x"]
				var/y2 = entry_position["y"] + 39
				var/middle_x = round((x1 + x2) / 2)
				line_html += "<path class='link [entry_state]' d='M [x1] [y1] C [middle_x] [y1], [middle_x] [y2], [x2] [y2]'/><circle class='joint [entry_state]' cx='[x2]' cy='[y2]' r='4'/>"
		return "<div class='graph-wrap'><div class='graph-canvas' style='width:[canvas_width]px;height:[canvas_height]px'>[lane_html][tier_html]<svg class='connections' width='[canvas_width]' height='[canvas_height]' viewBox='0 0 [canvas_width] [canvas_height]'>[line_html]</svg>[node_html]</div></div>"

	proc/buildHtml()
		owner.updatePassiveProgression()
		var/tabs = ""
		for(var/tab in list("Science", "Magic", "Mining", "Smithing", "Combat", "Milestones"))
			var/tab_state = tab == category ? "active" : ""
			tabs += "<a class='tab [tab_state]' href='byond://?src=\ref[src]&action=category&id=[tab]'>[uppertext(tab)]</a>"
		...
		return {"<!doctype html><html><head><meta charset='utf-8'><title>Progression Trees</title><style>[getNexusRpgBrowserCss()]
		*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:#070a0e;color:#e8edf2;font:12px Arial,sans-serif}body{background:radial-gradient(circle at 50% -10%,#26303b 0,#10161d 38%,#080b0f 72%);background-attachment:fixed}.shell{min-height:100vh}.header{position:sticky;top:0;z-index:5;border-bottom:1px solid #536170;background:rgba(8,12,17,.97);box-shadow:0 6px 18px #000;padding:12px 16px}.top{display:flex;align-items:center;gap:10px}.title{margin-right:auto}.title b{display:block;font:22px Georgia,serif;letter-spacing:2px;color:#f2f5f8}.title small{color:#8090a1;letter-spacing:.8px}.currency{padding:8px 12px;border:1px solid #d7a83e;background:#241c0b;color:#ffd970;font-weight:bold}.close{padding:8px 11px;border:1px solid #70474c;color:#ffaaa5;text-decoration:none}.tabs,.subtabs{display:flex;gap:5px;margin-top:10px}.tab,.subtab{padding:8px 12px;border:1px solid #354454;background:#101923;color:#8192a4;text-decoration:none;font-weight:bold}.tab.active,.subtab.active{border-color:#e0b341;background:#392b0c;color:#ffde79;box-shadow:inset 0 -2px #f4c64e}.subtabs{justify-content:center}.subtab{min-width:120px;text-align:center}.source{margin-top:9px;color:#718194;font-size:10px}.trees{display:flex;align-items:flex-start;justify-content:center;gap:18px;padding:20px;overflow-x:auto}.branch{min-width:270px;max-width:330px;flex:1}.branch h2{margin:0 0 12px;padding:10px;text-align:center;border:1px solid #556474;background:linear-gradient(#1b2530,#10171f);color:#d9e3ec;font:15px Georgia,serif;letter-spacing:1px}.rail{position:relative;padding:0 0 20px}.rail:before{content:'';position:absolute;left:38px;top:20px;bottom:40px;width:4px;background:linear-gradient(#e6b640,#9d6b17 70%,#343b42);box-shadow:0 0 8px #bd861f}.node{position:relative;display:grid;grid-template-columns:60px 1fr auto;gap:8px;min-height:92px;margin:0 0 14px;padding:9px 9px 9px 8px;border:1px solid #4d5a67;background:linear-gradient(135deg,#17212b,#0d131a);color:#dce5ed;text-decoration:none;box-shadow:0 4px 10px #000}.node:before{content:'';position:absolute;left:28px;top:-15px;width:22px;height:15px;border-left:4px solid #d9a62e;border-bottom:4px solid #d9a62e}.node:first-child:before{display:none}.node.available{border-color:#e2b33e;box-shadow:0 0 12px rgba(239,183,49,.34),0 4px 10px #000}.node.available:hover{background:linear-gradient(135deg,#29384a,#121c27);transform:translateY(-1px)}.node.owned{border-color:#d9ad3d;background:linear-gradient(135deg,#3a2d0d,#151612)}.node.locked{filter:grayscale(1);opacity:.53}.icon{position:relative;z-index:2;width:56px;height:56px;display:flex;align-items:center;justify-content:center;border:2px solid #be9131;background:#05070a;box-shadow:0 0 7px #000;overflow:hidden}.icon img{max-width:52px;max-height:52px;image-rendering:pixelated}.fallback{font:bold 20px Georgia,serif;color:#e6bb54}.copy{min-width:0}.copy b,.copy small,.copy em{display:block}.copy b{color:#ffe091;font-size:13px}.copy small{margin-top:5px;color:#aebac6;line-height:1.3;max-height:34px;overflow:hidden}.copy em{margin-top:6px;color:#7f91a2;font-size:9px;font-style:normal}.rank{align-self:start;padding:3px 5px;background:#05070a;color:#f2c34f;font-size:9px}.cost{position:absolute;right:7px;bottom:6px;color:#c99f3b;font-size:9px}.tier{position:absolute;left:72px;top:-7px;padding:1px 5px;background:#0a0e13;color:#6f8090;font-size:8px}.empty{padding:70px;text-align:center;color:#728191}@media(max-width:850px){.trees{justify-content:flex-start}.branch{min-width:280px}.tab{padding:7px 8px;font-size:10px}}
		.graph-wrap{min-height:560px;padding:18px;overflow:auto}.graph-canvas{position:relative;margin:0 auto;border:1px solid #35414d;background:linear-gradient(rgba(255,255,255,.018) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.018) 1px,transparent 1px),radial-gradient(circle at 35% 15%,rgba(50,75,93,.28),transparent 48%),#090d12;background-size:24px 24px,24px 24px,auto,auto;box-shadow:0 12px 32px #000}.branch-lane{position:absolute;z-index:0;left:8px;right:8px;border:1px solid rgba(91,112,130,.23);border-radius:18px;background:linear-gradient(90deg,rgba(27,39,49,.55),rgba(12,18,24,.2))}.branch-lane span{position:absolute;left:12px;top:8px;color:#667a8d;font:bold 10px Arial,sans-serif;letter-spacing:1.3px;text-transform:uppercase}.tier-marker{position:absolute;z-index:3;top:14px;width:122px;text-align:center;color:#708295;font:bold 9px Arial,sans-serif;letter-spacing:1.5px}.connections{position:absolute;z-index:1;left:0;top:0;overflow:visible;pointer-events:none}.link{fill:none;stroke:#3a4653;stroke-width:3}.link.locked{stroke:#2d353e}.link.available{stroke:#d8a938}.link.owned{stroke:#48c98c}.joint{fill:#3a4653}.joint.available{fill:#e8b840}.joint.owned{fill:#5bdd9e}.tree-node{position:absolute;z-index:4;width:122px;min-height:82px;text-align:center;color:#dce5ed;text-decoration:none}.node-icon{position:relative;display:flex;align-items:center;justify-content:center;width:58px;height:58px;margin:0 auto 5px;border:3px solid #485663;border-radius:50%;background:radial-gradient(circle,#1d2934,#05070a 72%);box-shadow:0 4px 10px #000;overflow:hidden}.node-icon img{max-width:52px;max-height:52px;image-rendering:pixelated}.tree-node b{display:block;height:28px;padding:0 2px;overflow:hidden;color:#d9e3ec;font-size:11px;line-height:13px;text-shadow:0 2px 2px #000}.node-meta{display:block;margin-top:2px;color:#718397;font-size:9px}.node-tier{position:absolute;z-index:2;left:72px;top:-3px;padding:2px 5px;border:1px solid #485663;border-radius:8px;background:#080b0f;color:#8b9bad;font:bold 8px Arial,sans-serif}.tree-node.locked{filter:grayscale(1);opacity:.45}.tree-node.available .node-icon{border-color:#e0b341;box-shadow:0 0 13px rgba(224,179,65,.52),0 4px 10px #000}.tree-node.available:hover .node-icon{transform:scale(1.07);background:radial-gradient(circle,#3a321a,#080a0d 72%)}.tree-node.owned .node-icon{border-color:#48c98c;box-shadow:0 0 13px rgba(72,201,140,.45),0 4px 10px #000}.tree-node.owned .node-tier{border-color:#48c98c;color:#7fe9b7}.tree-node.capstone .node-icon{border-color:#ff4e9a;background:radial-gradient(circle,#4a1831,#09070b 72%);box-shadow:0 0 18px rgba(255,78,154,.65),0 4px 10px #000}.tree-node.capstone .node-tier{border-color:#ff4e9a;color:#ff86bb}.node-tip{display:none;position:absolute;z-index:30;left:126px;top:-8px;width:240px;padding:10px;border:1px solid #a78336;background:rgba(5,8,11,.98);box-shadow:0 8px 22px #000;text-align:left;pointer-events:none}.tree-node:hover{z-index:25}.tree-node:hover .node-tip{display:block}.node-tip strong,.node-tip small,.node-tip em{display:block}.node-tip strong{color:#ffe091;font-size:12px}.node-tip small{margin-top:6px;color:#b2bec9;line-height:1.35}.node-tip em{margin-top:7px;color:#d0a83e;font-size:10px;font-style:normal}
		...
		.milestone-list{display:grid;grid-template-columns:repeat(auto-fill,minmax(270px,1fr));gap:12px;padding:20px}.milestone-card{position:relative;min-height:142px;padding:36px 14px 14px;border:1px solid #4d5a67;background:linear-gradient(135deg,#17212b,#0d131a);color:#dce5ed;text-decoration:none;box-shadow:0 4px 10px #000}.milestone-card strong,.milestone-card small,.milestone-card .milestone-status{display:block}.milestone-card strong{color:#ffe091;font:16px Georgia,serif}.milestone-card small{margin-top:9px;color:#aebac6;line-height:1.4}.milestone-branch{position:absolute;left:12px;top:10px;padding:3px 7px;border:1px solid #475767;background:#090d12;color:#8fa2b4;font-size:9px;letter-spacing:1px}.milestone-status{position:absolute;left:14px;right:14px;bottom:12px;color:#d0a83e;font-size:10px}.milestone-card.available{border-color:#e2b33e;box-shadow:0 0 12px rgba(239,183,49,.25),0 4px 10px #000}.milestone-card.available:hover{background:linear-gradient(135deg,#29384a,#121c27);transform:translateY(-1px)}.milestone-card.owned{border-color:#48c98c;background:linear-gradient(135deg,#173328,#101916)}.milestone-card.locked{filter:grayscale(1);opacity:.55}
		.branch-lane{border-radius:18px!important}.node-icon{border-radius:50%!important;box-shadow:0 4px 10px #000!important}.node-tier{border-radius:8px!important}.tree-node.available .node-icon{box-shadow:0 0 13px rgba(224,179,65,.52),0 4px 10px #000!important}.tree-node.owned .node-icon{box-shadow:0 0 13px rgba(72,201,140,.45),0 4px 10px #000!important}.tree-node.capstone .node-icon{box-shadow:0 0 18px rgba(255,78,154,.65),0 4px 10px #000!important}.node-tip{box-shadow:0 8px 22px #000!important}.node-tip small{color:#b2bec9!important}
		</style><script>function runTreeSearch(){var q=document.getElementById('tree-search').value||'';window.location.href='byond://?src=\ref[src]&action=search&q='+encodeURIComponent(q);return false;}function jumpTier(tier){var wrap=document.querySelector('.graph-wrap');if(!wrap)return;wrap.scrollLeft=Math.max(0,70+(tier-1)*190-wrap.clientWidth/2);}</script></head><body><div class='shell'><div class='header'><div class='top'><div class='title'><b>PROGRESSION / [uppertext(category)]</b><small>Load one branch at a time or search across the category</small></div><span class='currency'>[currency_label]</span><a class='close' href='byond://?src=\ref[src]&action=close'>CLOSE</a></div><div class='tabs'>[tabs]</div><div class='source'>Earned: [round(owner.progression_chat_experience, 0.1)] from [owner.progression_roleplay_sessions_completed] roleplay sessions / [round(owner.progression_passive_experience, 0.1)] from active time / [round(owner.progression_lifetime_experience, 0.1)] lifetime</div>[tree_navigation]</div><main>[buildGraph()]</main></div></body></html>"}
	...
	Topic(href, list/href_list)
		...
		switch(href_list["action"])
			if("category")
				if(href_list["id"] in list("Science", "Magic", "Mining", "Smithing", "Combat", "Milestones"))
