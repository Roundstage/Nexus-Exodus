	...
	while(1)
		...
		for(var/obj/Wall_upgrader_bot/b in wall_bots) if(b.z)
			...
			for(var/turf/t in range(1,b)) if(t.Builder&&!(t.Builder in upgraded))
				upgraded+=t.Builder
				t.upgradeAll(temp,display_message=0,for_free=1)
		if(temp) del(temp)
		sleep(1*60*60*10)
...
mob/proc/LSD()
	...
	spawn(40) while(src&&LSD)
		if(prob(70)) src<<pick('CreepyAmbience.ogg','CreepyAmbience2.ogg')
		else
			src<<pick('ScaryGirl.ogg','Wisp.ogg','DistantMonsterRoar.ogg')
...
obj/items/Senzu
	...
	New()
		pixel_x=rand(-8,8)
		pixel_y=rand(-8,8)
		//icon=pick('FoodTomato.dmi','FoodTurnip.dmi','FoodPotato.dmi',\
		//'FoodJungleFruit.dmi','FoodGrapes.dmi','FoodCorn.dmi')
		senzus+=src
		. = ..()
