...
var
	ssj_unmastered_eye_overlay
	ssj_mastered_eye_overlay
...
proc/getSsjEyeOverlay(mastered)
	...
	if(mastered)
		...
		eyes.MapColors("#4aa7ff", "#dcefff", "#071426")
		ssj_unmastered_eye_overlay = eyes
	return eyes

mob/proc/currentSsjFormMastered()
	if(ssj == 1) return ssjdrain >= max_ss_mastery
	if(ssj == 2) return ssj2drain >= max_ss_mastery
	if(ssj == 3) return ssj3drain >= max_ss_mastery
	if(ssj == 4) return TRUE
	return FALSE
...
mob/proc/SSj() if(!transing&&!ssj&&!IsGreatApe())
	preparePrimaryTransformation("saiyan_ssj1")
	...
	transing=0
	ssj_drain_loop()
	syncActivePrimaryTransformation("ssj1")
...
mob/proc/SSj2() if(!transing&&ssj==1&&Class!="Legendary Saiyan")
	...
	ssj_power+=ssj2add
	transing=0
	syncActivePrimaryTransformation("ssj2")
...
mob/proc/SSj3() if(!transing&&ssj==2)
	...
	ssj_bp_mult*=form3x
	transing=0
	syncActivePrimaryTransformation("ssj3")
mob/var
	SSj4Able
	ssj4hair
...
mob/proc/SSj4() if(!IsGreatApe() && !transing && !ssj && !ismystic)
	if(IsGod()) return
	preparePrimaryTransformation("saiyan_ssj4")
	transing=1
	ssj=4
...
mob/proc/Frost_Lord_Forms() if(Race=="Frost Lord")
	if(!Form && !is_gold_form) preparePrimaryTransformation("frost_second")
	if(Form == 4 && IsCooler) PowerUpToGoldForm()
	else if(Form == 3 && !IsCooler) PowerUpToGoldForm()
	...
	if(!Form)
		...
		icon=Form2Icon
		//overlays-=overlays
	syncActivePrimaryTransformation("frost advance")
...
mob/proc/icer_Revert()
	...
	if(Form==4)
		...
		Form=3
		recov*=icer_recovery
	syncActivePrimaryTransformation("frost revert")
