...
mob/proc
	...
	armStretchRangeTiles(range_pixels)
	...
	GetArmStretchTarget(grab_dist = 10)
		...
		var/mob/selected_mob = getSelectedTarget(max_dist = grab_dist, dir_angle = dir, angle_limit = 50)
		if(selected_mob && CanExtendoGrab(selected_mob)) return selected_mob

		var/list/targets = GetArmStretchTargets(grab_dist, loc, dir)
		var/list/targets2 = GetArmStretchTargets(grab_dist, loc, turn(dir, 45))
		var/list/targets3 = GetArmStretchTargets(grab_dist, loc, turn(dir, -45))
		...
		if(mobs.len)
			for(var/mob/m in mobs)
				...
				else if(bounds_dist(src,m) < bounds_dist(src,preferred_mob)) preferred_mob = m
		...
		for(var/obj/o in targets)
			...
			else if(bounds_dist(src,o) < bounds_dist(src,preferred_obj)) preferred_obj = o
	...
	Grab_failed(atom/movable/m, list/arms, step_number=0, grab_dist=0, obj/last_arm, turf/starting_loc)
	...
	Get_next_arm_position(obj/old_arm,atom/movable/m)
	...
	Stretch_arm_to(atom/movable/m,grab_dist=10)
		...
		for(var/step_number in 1 to grab_dist)
			...
			arm.icon=arm_icon
			arm.step_x = step_number == 1 ? step_x : 0
			arm.step_y = step_number == 1 ? step_y : 0
		...
		while(arms.len)
			...
			if(!last_arm.z) return
				...
				if(last_arm)
					...
					if(m && !isturf(m) && is_grabbing && m.z == z)
						m.SafeTeleport(last_arm.loc)
						m.step_x = last_arm.step_x
						m.step_y = last_arm.step_y
