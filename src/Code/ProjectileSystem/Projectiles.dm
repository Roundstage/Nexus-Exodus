...
obj/Blast/proc/setStats(mob/P, Percent=1, Off_Mult=1, Explosion=0, bullet=0, homing_mod = 1, explosion_percent = 0, max_damage_factor = 0, owner_immunity = 0, datum/CombatDamageBudget/shared_budget)
	...
	explosion_damage_factor = explosion_percent
	owner_immune = owner_immunity
...
obj/Blast
	...
	proc/reserveDamageFactor(mob/target, requested_factor)
		if(damage_budget) return damage_budget.reserveFactor(target, requested_factor)
	...
	proc/Shield(mob/A, requested_factor)
		...
		var/factor = reserveDamageFactor(A, requested_factor)
		if(factor <= 0) return
		var/scaled_damage = getProjectileCombatDamage(A, factor)
		for(var/obj/items/Force_Field/S in A.item_list) if(S.Level>0)
			var/dmg = scaled_damage * A.BP * 0.0017 * shield_pierce_mult
	...
	proc/getBeamDamageWindow(loop_delay)
	...
	proc/Beam()
		...
		while(src)
			...
			Damage = BP * Force * percent_damage * damage_window
			if(!Owner)
				del(src)
				return
			/*if(ismob(Owner))
			...
			if(!br)
				...
				for(var/mob/A in range(beam_radius,src)) if((A.loc==loc || A!=Owner) && (A.loc==loc || get_dir(src,A) != dir))
					...
					if(A.ultra_instinct)
						...
						if(!A.regenerator_obj && A.Shielding())
							...
							var/shield_factor = percent_damage
							if(beam_impact_mode == BEAM_IMPACT_LOCK) shield_factor *= damage_window / beam_raw_damage_mod
							Shield(A, shield_factor)
						...
						if(A && !A.regenerator_obj && A.precog && A.precogs && prob(A.precog_chance) && !A.KO && (A.Flying || A.icon_state == "") && A.Ki > A.max_ki * 0.2 && !A.Disabled())
							...
							var/requested_factor = percent_damage
							if(beam_impact_mode == BEAM_IMPACT_LOCK)
								requested_factor *= damage_window / beam_raw_damage_mod
							var/factor = reserveDamageFactor(P, requested_factor)
							var/dmg = getProjectileCombatDamage(P, factor)
							...
							var/atom/impact_target = P
							if(!impact_target) impact_target = A
							if(showExplosiveBeamImpact(impact_target, force_mob_impact = 1)) return
							if(!Piercer)
								icon_state="struggle"
					...
					if(A.dir != dir)
						if(!A.Beam)
							...
							if(3*BP*Force*percent_damage**0.3/Beam_Delay>A.BP*A.Force*A.percent_damage**0.3)
								...
								A.startKiProjectileWalk(pick(dirs))
							else
				...
				if(t && isturf(t) && t.density)
					...
					if(t) if(!Piercer && t.density)
						if(showExplosiveBeamImpact(t)) return
						icon_state="struggle"
						delete_on_next_move=1
						//Owner.ToTens("8")

				for(var/obj/A in loc) if(!istype(A,/obj/Blast) && !istype(A,/obj/Edges))
					var/object_was_dense = A.density
					if(!A.takes_gradual_damage)
						...
						if(A.Health<=0)
							...
							del(A)
					if(!Piercer && object_was_dense)
						if(showExplosiveBeamImpact(A ? A : loc)) return
		...
		for(var/atom/movable/am in view(Explosive,src))
			if(ismob(am)&&am!=Owner)
				...
				if(m.attack_barrier_obj&&m.attack_barrier_obj.Firing_Attack_Barrier)
					...
					if(!m.regenerator_obj && m.Shielding()) Shield(m, explosion_damage_factor)
					else
						var/kb_dist=Explosive+Shockwave
						kb_dist=m.relative_kb_dist(src,kb_dist)
						m.Shockwave_Knockback(kb_dist,loc)
						var/factor = reserveDamageFactor(m, explosion_damage_factor)
						var/dmg = getProjectileCombatDamage(m, factor)
	...
	proc/BlastMobCross(mob/m, override_dir, override_delete)
		...
		if(!m.regenerator_obj)
			if(m.Shielding())
				Shield(m, percent_damage)
				player_view(10,src) << sound('Reflect.ogg',volume = 10)
				Size = 0
				deflected = 1
				blast_go_over_owner = 0
				startKiProjectileWalk(pick(NORTH,SOUTH,EAST,WEST,NORTHWEST,NORTHEAST,SOUTHWEST,SOUTHEAST))
				return 1
			else
	...
	proc
		Bounce_Dir()
			set waitfor=0
			if(Bounce) startKiProjectileWalk(pick(turn(dir,135),turn(dir,-135),turn(dir,180)))
