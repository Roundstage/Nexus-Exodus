...
obj/Attacks/var
	...
	damage_factor = 0
	tmp/datum/CombatDamageBudget/beam_damage_budget
...
mob/var/beam_impact_mode = BEAM_IMPACT_EXPLOSIVE

obj/var/tmp
	list/beam_objects = new
	beam_delay=1
	immediate_beam_teardown
		...
		if(!m.beaming && !beam_objects.len)
			...
			for(var/obj/Blast/o in beam_objects) if(!(locate(/obj/beam_redirector) in o.loc))
				if(!(o.icon_state in list("blank")))
					o.Beam_Appearance()
			if(immediate_beam_teardown) l = new/list
			beam_objects = l
...
mob/proc/BeamStreamLoop(obj/Attacks/A)
	...
	while(A.streaming&&src)
		...
		if(can_generate_beam_segment)
			...
			B.density=0
			B.Beam=1
			B.beam_impact_mode = beam_impact_mode
			B.Piercer=A.Piercer
			B.dir=dir
			B.BP=BP
			B.setStats(src, A.damage_factor, Off_Mult = 1, Explosion = 0, shared_budget = A.beam_damage_budget)
			B.from_attack=A
	...
	/*spawn for(var/i=my_beam_objs.len,i>0,i--)
		if(my_beam_objs.len>=i)
			...
			if(o && o.z)
				del(o)*/
	if(immediate)
		A.immediate_beam_teardown = 1
		deleteBeamSegmentsImmediately(A, impact_segment)
	else BeamStopThing2() //i dont have a good name for this but it replaces the commented out chunk above
	//beam_struggling=0
	beaming=0
	charging_beam=0
	A.charging=0
...
mob/proc/deleteBeamSegmentsImmediately(obj/Attacks/A, obj/Blast/impact_segment)
	var/list/segments_to_delete = list()
	if(my_beam_objs) segments_to_delete |= my_beam_objs
	if(A && A.beam_objects) segments_to_delete |= A.beam_objects
	segments_to_delete -= impact_segment
	my_beam_objs = new/list
	if(A) A.beam_objects = new/list
	for(var/obj/Blast/segment in segments_to_delete)
		if(!segment) continue
		segment.beam_loop_running = 1
		segment.SafeTeleport(null)
		segment.DeleteNoWait()
