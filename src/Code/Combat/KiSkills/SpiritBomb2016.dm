...
mob/proc
	...
	SpiritBombThrow(obj/Attacks/Genki_Dama/sb)
		...
		player_view(10,src) << sound('BasicbeamFire.ogg',volume=15)
		var/controlled = 0
	...
	SpiritBombBegin(obj/Attacks/Genki_Dama/sb)
		...
		CenterIcon(b)
		b.setStats(src, Percent = sb.sb_initial_dmg, Off_Mult = 10, Explosion = sb.sb_explosion_size, explosion_percent = sb.sb_explosion_size ? sb.sb_initial_dmg : 0, max_damage_factor = sb.sb_max_dmg * (sb.sb_explosion_size ? 2 : 1))
		b.from_attack = sb
	...
	SpiritBombPowerGrow(obj/Blast/Genki_Dama/b, obj/Attacks/Genki_Dama/sb)
		...
		var
			...
			charge_start = timer * 0.25
			elapsed_time = 0
		while(elapsed_time < timer)
			var/release_progress = Clamp((elapsed_time - charge_start) / max(timer - charge_start, 0.01), 0, 1)
			var/dmg = sb.sb_initial_dmg + (sb.sb_max_dmg - sb.sb_initial_dmg) * release_progress
			b.setStats(src, Percent = dmg, Off_Mult = sb.sb_deflect_difficulty, Explosion = sb.sb_explosion_size, explosion_percent = sb.sb_explosion_size ? dmg : 0)
			sleep(TickMult(3))
			elapsed_time += 3
