...
proc/initializeNexusAdminActions()
	...
	nexus_admin_action_catalog["give_rare_race"] = new /datum/NexusAdminAction("give_rare_race", "Give Rare Race", "Character", "Grant the selected account Legendary Saiyan, Frost Lord, Cooler, Grand Regent, Royal Blood (Argal), or all listed rare options.", 3, TRUE, TRUE)
	nexus_admin_action_catalog["unlock_argal"] = new /datum/NexusAdminAction("unlock_argal", "Unlock Argal Bloodline", "Character", "Unlock Royal Blood for character creation or convert a standard Viltrumite with a free stat redistribution.", 3, TRUE, TRUE)
...
datum/NexusAdminPanel
	...
	proc/openItemPicker(mode = "give", search = "")
		...
		for(var/item_type in typesof(/obj/items))
			...
			results += "<a class='result' data-search='[encodeNexusHtmlAttribute(lower_path)]' href='byond://?src=\ref[src]&action=create_item&index=[index_key]'><b>[html_encode(path_text)]</b><span>[category]</span></a>"
	...
	proc/runAction(action_id)
		...
		switch(action_id)
			...
			if("give_rare_race") call(owner, /mob/Admin3/verb/giveRareRace)(target)
			if("unlock_argal") call(owner, /mob/Admin3/verb/unlockArgalBloodlineForPlayer)(target)
	...
	proc/buildHtml()
		...
		for(var/category in category_order)
			for(var/action_id in nexus_admin_action_catalog)
				...
				action_html += "<a class='[card_class]' data-category='[action.category]' data-search='[encodeNexusHtmlAttribute(search_key)]' href='byond://?src=\ref[src]&action=run&id=[action.id]'><span class='hud-label'>[html_encode(action.category)] / LV [action.minimum_level]</span><b>[html_encode(action.name)]</b><small>[html_encode(action.description)]</small></a>"
