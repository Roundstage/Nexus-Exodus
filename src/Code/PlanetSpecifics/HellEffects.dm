...
mob/proc/Scary_Effects() while(1)
	if(z==6&&!Hell_Immune())
		if(client) switch(pick(2,3,4))
			...
			if(2) src<<pick('CreepyAmbience.ogg','CreepyAmbience2.ogg')
			if(3) src<<pick('ScaryGirl.ogg','Wisp.ogg','DistantMonsterRoar.ogg')
...
mob/proc/Freddy_Kreuger_Image()
	src<<sound(0)
	src<<sound('RingAroundTheRosey.ogg')
	sleep(rand(100,200))
...
mob/Admin4/verb/hellTorture()
	set name = "Hell Torture"
	Hell_Torture=input("[Hell_Torture]x") as num
