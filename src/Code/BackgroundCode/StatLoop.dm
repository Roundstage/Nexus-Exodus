...
mob/proc
	...
	racialDamageTakenMult()
		...
		switch(Race)
			if("Android") return android_dmg_taken_mult
			if("Bio-Android") return bio_android_dmg_taken_mult
			if("Majin") return majin_dmg_taken_mult
		return 1
...
mob
	...
	proc
		effectiveBaseBPMult()
			return feat_bp_multiplier * racialCombatBPMult()
...
mob/proc/get_bp(factor_powerup=1)
	...
	if(Tournament && skill_tournament && (src in All_Entrants))
		...
		switch(Race)
			if("Android") n*=1.5
			if("Majin") n*=1.2
			if("Demon") n *= 1 + (min(CollectedSouls, 3) * 0.1)
		if(client)
			...
			if(istype(a,/area/Braal_Core))
				...
				spawn if(src && client)
					...
					if(t)
						...
						//	spawn(rand(0,9)) t2.TempTurfOverlay('CrackFire.dmi',rand(15,25))
...
mob/proc/death_regen(set_loc=1)
	...
	if(!scrap_repair() || Scraps_Exist())
		...
		if(scrap)
			...
			del(scrap)
		else if(!scrap_repair()) SafeTeleport(locate(death_regen_loc[1],death_regen_loc[2],death_regen_loc[3]))
...
mob/proc
	...
	hp_ki_bp_loss_mult()
		...
		var
			max_loss = 0.99 //= lose 70% of your bp
