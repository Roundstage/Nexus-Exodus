...
mob/Admin3/verb/GiveScienceLevel(mob/M in world)
	...
	if(!M) return
	if(!M.client) return
	var/level = input(src, "Give [M] a science level.") as num
	if ((level*1) > 4 && !M.player_tech_paths)
		src << "The player doesn't have a path"
		return

	M.player_tech_level = (level*1)

	var/list/current_tech_list = list()
	for(var/obj/item in tech_list)
		if(!(item in M.individual_science_items) && item.science == 1 && M.player_tech_level >= item.science_level) 
			current_tech_list += item
	for(var/obj/tech in current_tech_list)
		if(!tech) return
		if(!(tech in GLOBAL_SCIENCE_TAB_ITEMS))
			if(tech.science_level >= 5 && tech.science_path in M.player_tech_paths)
				M.individual_science_items += tech
				M << "You have been given access to [tech] in the science tab."
			else if(tech.science_level < 5)
				M.individual_science_items += tech
				M << "You have been given access to [tech] in the science tab."
...
mob/Admin3/verb/GiveSciencePath(mob/M in world)
	...
	if(!M) return
	if(!M.client) return
	var/option = alert(src, "Give [M] a science path.", "Science Paths" ,"Genetics", "Engineering", "Robotics")
	if(option in M.player_tech_paths)
		src << "The player already have this path" 
		return
	M.player_tech_paths += option;
	M << "You were given [option] path";
